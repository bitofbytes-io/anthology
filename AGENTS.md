...
- The UI container's `Docker/ui/nginx.conf` intentionally serves the SPA shell only for allowlisted Angular routes and any documented compatibility aliases, then returns 404 for unknown direct paths; review it alongside `web/src/app/app.routes.ts` when changing routes.
